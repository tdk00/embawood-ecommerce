        ...
        <form action="{{ isset($subIdea) ? route('admin.sub-ideas.update', $subIdea->id) : route('admin.sub-ideas.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select class="form-control" name="idea_id" id="idea_id">
                    @foreach($ideas as $idea)
                        <option value="{{ $idea->id }}" {{ (isset($subIdea) && $subIdea->idea_id == $idea->id) || (isset($selectedIdeaId) && $selectedIdeaId == $idea->id) ? 'selected' : '' }}>
                            {{ $idea->title_category_view }}
                        </option>
                    @endforeach
                </select>
                @error('idea_id')
            ...
            <div class="mb-3">
                ...
                @if (isset($subIdea) && $subIdea->image_category_view)
                    <img src="{{ Storage::url('sub-ideas/' . $subIdea->image_category_view) }}" alt="" width="100">
                @endif
            </div>
            ...
            <div class="mb-3">
                ...
                @if (isset($subIdea) && $subIdea->image_homepage_tab_view)
                    <img src="{{ Storage::url('sub-ideas/' . $subIdea->image_homepage_tab_view) }}" alt="" width="100">
                @endif
            </div>
