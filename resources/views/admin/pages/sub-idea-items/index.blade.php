...
@section('title', 'SubIdeaItem List')

@section('content')
    <div class="container">
        <h1>SubIdeaItem List</h1>

        @if (session('success'))
            <div class="alert alert-success">{{ session('success') }}</div>
        @endif

        <a href="{{ route('admin.sub-idea-items.create') }}" class="btn btn-primary mb-4">Add New SubIdeaItem</a>
        ...
        <table class="table table-bordered">
            ...
            @foreach($subIdeaItems as $subIdeaItem)
                <tr>
                    ...
                    <td>{{ $subIdeaItem->subIdea->title ?? 'No SubIdea' }}</td>
